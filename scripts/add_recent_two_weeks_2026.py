# -*- coding: utf-8 -*-
"""
为 11 城补充 2026-08-12 ~ 2026-08-26（最近两周）真实活动。

数据来源：/workspace/recent_events_<city>.json（由调研子代理经 WebSearch/WebFetch
真实核实、带可溯源链接采集，严格不编造）。

处理方式：
- 读取各城调研结果，按项目 rec() 格式补全省略字段（city/link/url/
...
def main():
    ...
    for fp in sorted(glob.glob(os.path.join(SRC_DIR, 'recent_events_*.json'))):
        city_en = os.path.basename(fp)[len('recent_events_'):-len('.json')]
