...
# 城市反查：优先用场馆注册表精确匹配，兜底用城市关键词启发式。
# 这是分城市文件(output/exhibitions_{city}.json)正确生成的前提，
# 否则 generate_city_exhibitions.py 会因活动缺 city 字段而生成 0 个文件。
try:
    from venue_registry import lookup_venue_by_name as _lookup_venue
except ImportError:
    _lookup_venue = None

_CITY_KEYWORDS = {
    'shenzhen': ['深圳', '宝安', '南山', '福田', '罗湖', '龙岗', '龙华', '光明', '坪山', '盐田', '大鹏', '前海'],
    'guangzhou': ['广州', '天河', '越秀', '海珠', '番禺', '白云'],
    'shanghai': ['上海', '浦东', '徐汇', '黄浦', '静安'],
    'beijing': ['北京', '朝阳', '海淀', '东城', '西城'],
    'hangzhou': ['杭州', '西湖', '余杭', '滨江'],
    'chengdu': ['成都', '武侯', '锦江', '青羊'],
    'chongqing': ['重庆', '渝中', '江北'],
    'wuhan': ['武汉', '武昌', '洪山', '江汉'],
    'xian': ['西安', '雁塔', '碑林'],
...
def _get_city(text):
    """推断场馆/来源所属城市代码；优先注册表精确匹配，兜底关键词启发式。"""
    if not text:
        return ''
    if _lookup_venue:
        try:
            v = _lookup_venue(str(text))
            if v and v.get('city'):
                return v['city']
        except Exception:
            pass
...
def _get_district_from_registry(text):
    """优先用注册表拿到场馆的精确区县（比关键词启发式更可靠）。"""
    if not text or not _lookup_venue:
        return None
    try:
        v = _lookup_venue(str(text))
        if v and v.get('district'):
            return v['district']
    except Exception:
        pass
    return None
...
def normalize_activity(raw, venue_default=''):
    ...
    # 城市反查（注册表优先，关键词兜底）—— 分城市文件生成的关键
    city = _get_city(venue) or _get_city(source)
    # 区县优先用注册表精确值，否则保留关键词启发式结果
    reg_district = _get_district_from_registry(venue)
    if reg_district:
        dist_venue = reg_district
    ...
    result = {
        ...
        'source': source,
        'city': city,
        'district': dist_venue if dist_venue else '',
    }
