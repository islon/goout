...
const APP_VERSION = '2026.07.18.15';
...
App({
  ...
  onLaunch() {
    ...
    // 5.5 开发者工具专用：本机若跑企业 SSL 代理(如 aTrust)，开发者工具的 Chromium 到 jsDelivr/raw
    //     的 HTTPS 握手会被重置(net_error -101)，远程场馆拉取失败、长期停在打包兜底(378)。
    //     开发者工具本地运行不限制主包大小，故直接用内置全量场馆文件(上传时由 packOptions.ignore 剔除)，
    //     让本地验证场馆功能无需联网。真机 platform 非 'devtools'，走正常远程拉取，不受影响。
    try {
      var _sys = wx.getSystemInfoSync();
      if (_sys && _sys.platform === 'devtools') {
        var _dev = require('./data/venues_dev.js');
        if (_dev && _dev.length) {
          this.globalData.venues = _dev;
          this.globalData.venuesPartial = false;
          this.buildVenueMap();
          try {
            wx.setStorageSync(VENUE_CACHE_KEY, _dev);
            wx.setStorageSync(VENUE_PARTIAL_KEY, false);
          } catch (e) {}
          console.log('[童行][devtools] 使用本地全量场馆', _dev.length, '条（绕过网络/SSL，便于本地验证）');
        }
      }
    } catch (e) {}
